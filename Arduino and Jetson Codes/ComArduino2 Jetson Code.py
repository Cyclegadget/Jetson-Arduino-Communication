...
#  the user must modify the code to include the correct reference.
...
def recvFromArduino():
    ...
    byteCount = -1  # to allow for the fact that the last increment
...
# ======================================
def myAppender(orders):
    b = []
    b = orders.encode('utf-8')
    testData.append(b)
    return testData     
...
def runTest(td):
    ...
    while n < numLoops:
      ...
      if waitingForReply == False:
        ...
        print(n + 1)                #loop count corrected
        print(" TEST STR ")
        print(teststr)
        n += 1
     ...
     # while ser.inWaiting() <= 0:###########
        ...
        print(dataRecvd)  
        ################the begining of parsing received data     
        b = (dataRecvd[4:8]) 
        print(b)
        b = (dataRecvd[18:21]) 
        print(b)
...
# ======================================
# #serPort = "/dev/ttyS80" or ACM1, ACM0 ensure serial port, baudrate are correct
...
#Here the program is using myAppender() to append "testData" to then send to the Arduino
myAppender("<LED1,200,0.2>")
myAppender("<LED1,800,0.7>")
myAppender("<LED2,800,0.5>")
myAppender("<LED2,200,0.2>")
myAppender("<LED1,200,0.7>")
myAppender("<LED2,200,0.2>")
myAppender("<LED1,200,0.7>")
